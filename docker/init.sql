...
CREATE TABLE `mysterious_report` (
    ...
    `region` varchar(255) NOT NULL DEFAULT '' COMMENT '执行区域快照',
    `service_name` varchar(128) NOT NULL DEFAULT '' COMMENT '执行时服务名快照',
    `total_threads` int NOT NULL DEFAULT '0' COMMENT '执行时总线程数快照',
    `slave_count` int NOT NULL DEFAULT '0' COMMENT '执行时压力机数快照',
    `grafana_instance` varchar(255) NOT NULL DEFAULT '' COMMENT '执行时Grafana instance快照',
    `artifact_dir` varchar(255) NOT NULL DEFAULT '' COMMENT '执行产物目录快照',
    `creator_id` varchar(32) NOT NULL DEFAULT '' COMMENT '创建人ID',
    `creator` varchar(32) NOT NULL DEFAULT '' COMMENT '创建人',
    `modifier_id` varchar(32) NOT NULL DEFAULT '' COMMENT '修改人ID',
    `modifier` varchar(32) NOT NULL DEFAULT '' COMMENT '修改人',
