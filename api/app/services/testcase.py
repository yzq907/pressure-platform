...
async def run_testcase(db: AsyncSession, id: int, param: RunParam, user: UserContext) -> bool:
    ...
    # 检查其它用例不在运行（Phase 5 跳过 Redis 排队，直接抛）
    running = await crud.list_by_status(db, TestCaseStatus.RUN_ING.value)
    if running:
        raise MysteriousException(Codes.TESTCASE_IS_RUNNING)

    # 检查所有 enabled slaves 都能 telnet 通（替代 Java 的 PingUtil）
    enable_slaves = await node_crud.list_enable_slaves(db)
...
async def stop_testcase(db: AsyncSession, id: int, user: UserContext) -> bool:
    ...
    bin_home = await config_service.get_value(db, MASTER_JMETER_BIN_HOME_KEY)
    cmd = [os.path.join(bin_home, "shutdown.sh")]
    log.info("[stop] cmd=%s", " ".join(cmd))
    await jmeter_runner.launch_stop(cmd, testcase_id=id)
    return True
